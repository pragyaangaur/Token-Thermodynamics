...
Neither step downloads a model, so the whole workflow finishes in under a minute. It runs on Python 3.12 and 3.13 so that a version problem in the pinned dependencies is caught rather than left for a reader to hit.
